...
import { nextLiveTemplateId, renderLiveFlyer, resolveLiveTemplateId } from '@/lib/live-flyer'
import {
...
async function saveAutoGeneratedFlyer(
  ...
  const templateId = resolveLiveTemplateId(topic, liveDate, requestedTemplateId)
  const templateResponse = await fetch(new URL(`/live-templates/${templateId}.png`, baseUrl), { cache: 'no-store' })
  if (!templateResponse.ok) throw new Error(`No se pudo cargar la plantilla ${templateId} del Live.`)
  const templateBuffer = Buffer.from(await templateResponse.arrayBuffer())
  const rendered = await renderLiveFlyer({ topic, liveDate, requestedTemplateId: templateId, templateBuffer })
  ...
  const storagePath = `copy-requests/${copyId}/v${version}-${Date.now()}-live-auto-${rendered.templateId}.png`
  ...
  const { data: asset, error: assetError } = await admin
    ...
    .insert({
      ...
      created_by: userId,
      metadata: {
        original_name: `live-${liveDate}-${rendered.templateId}.png`,
        mime_type: 'image/png',
        size: rendered.buffer.length,
        source: 'crm_live_auto_renderer',
        media_type: 'image',
    ...
    .select('id,asset_type,public_url,metadata,version,status')
    .single()
  if (assetError) throw assetError

  return { asset, templateId: rendered.templateId }
}
...
export async function POST(
  ...
  try {
    ...
    if (payload.action === 'configure') {
      ...
      const requestedTemplateId = String(payload.templateId || 'auto').trim()
      ...
      const flyer = await saveAutoGeneratedFlyer(
        admin,
        id,
        ctx.userId,
        topic,
      ...
      if (error) throw error
      return NextResponse.json({ settings, groups, asset: flyer.asset, flyerGenerated: true })
    }

    if (payload.action === 'regenerate-flyer') {
      const settings = await getSettings(admin, id)
      if (!settings) {
        return NextResponse.json({ error: 'Primero configura el Live.' }, { status: 400 })
      }
      const topic = cleanLiveTopic(ctx.item.product_topic)
      const requestedRaw = String(payload.templateId || 'next').trim()
      const requested = requestedRaw === 'next' ? nextLiveTemplateId(settings.template_id) : requestedRaw
      const flyer = await saveAutoGeneratedFlyer(
        admin,
        id,
        ctx.userId,
      ...
      const { data: updatedSettings, error: settingsError } = await admin
        .from('copy_live_settings')
        .update({ template_id: flyer.templateId })
        .eq('copy_request_ref', id)
        .select('*')
        .single()
      if (settingsError) throw settingsError
      return NextResponse.json({ settings: updatedSettings, asset: flyer.asset, flyerGenerated: true })
    }
    ...
    const asset = await getSelectedAsset(admin, id)
    if (!asset?.public_url) {
      return NextResponse.json({ error: 'No se encontró el flyer automático. Regénéralo antes de aprobar y programar.' }, { status: 400 })
    }
