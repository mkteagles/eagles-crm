...
export function LeadsTable() {
  ...
  if (loading) return <div className="p-4">Cargando leads...</div>
  if (error) return <div className="p-4 text-red-500">Error: {error}</div>
  if (leads.length === 0) return <div className="p-4 text-foreground/50">No hay leads con estos filtros.</div>
      ...
      <table className="w-full border-collapse">
        <thead>
          <tr className="bg-foreground/5">
            ...
            <th className="p-3 text-left text-sm font-semibold">Engagement</th>
            <th className="p-3 text-left text-sm font-semibold">¿Compró?</th>
            <th className="p-3 text-left text-sm font-semibold">Acción</th>
          </tr>
        </thead>
        <tbody>
          {leads.map((lead: Lead) => (
            <tr key={lead.id} className="border-b border-border-color hover:bg-foreground/5">
              ...
              <td className="p-3">
                ...
                >
                  <Phone size={16} /> {lead.phone_number}
                </a>
              </td>
              <td className="p-3">{lead.email || '—'}</td>
              <td className="p-3 text-sm text-foreground/60">{lead.campaign_name || '—'}</td>
              <td className="p-3">
                <span
              ...
              <td className="p-3 font-bold">{lead.score}/100</td>
              <td className="p-3">
                <span
                  className={
              ...
              <td className="p-3">
                <Link href={`/app1/leads/${lead.id}`} className="text-brand-blue hover:underline">
                  Ver más
                </Link>
              </td>
            </tr>
